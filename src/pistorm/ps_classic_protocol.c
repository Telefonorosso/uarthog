...
void ps_write_32_int(unsigned int address, unsigned int value)
{
    if (address & 1)
    {
...
/* POC6: implemented by src/raspi/start_rpi64.c.  This performs one bounded
 * DWC2 service pass and emits no synthetic USB traffic. */
extern void emu68_usb_housekeeper_poll(void);
...
void ps_housekeeper() 
{
    ...
    /* USB is serviced at about 1 kHz.  Reading the DWC2 status registers does
     * not itself generate any USB packets on the wire. */
    const uint64_t usb_poll_ticks = (tmp >= 1000) ? (tmp / 1000) : 1;
    uint64_t usb_last_poll;
    asm volatile("mrs %0, CNTPCT_EL0":"=r"(usb_last_poll));
    ...
    for(;;) {
        uint64_t usb_now;
        asm volatile("mrs %0, CNTPCT_EL0":"=r"(usb_now));
        if ((usb_now - usb_last_poll) >= usb_poll_ticks) {
            usb_last_poll = usb_now;
            emu68_usb_housekeeper_poll();
        }

...
extern void emu68_usb_putc(char c);

static void putByteMux(void *io_base, char chr)
{
    putByte(io_base, chr);
    emu68_usb_putc(chr);
}

#define KPRINTF_PUTC putByteMux
#else
#define KPRINTF_PUTC putByte
#endif
...
void __printf_chk(int, const char *restrict format, ...)
{
    ...
    vkprintf_pc(KPRINTF_PUTC, (void *)ARM_PERIIOBASE, format, v);
...
void vkprintf(const char * restrict format, va_list args)
{
    ...
    vkprintf_pc(KPRINTF_PUTC, (void*)ARM_PERIIOBASE, format, args);
...
/*
 * Minimal firmware power-domain helper used by the bare-metal USB POC.
 * device_id 3 is the legacy USB HCD/OTG power domain on BCM283x.
 * state bit 0 = on, bit 1 = wait for completion.
 */
uint32_t set_power_state(uint32_t device_id, uint32_t state)
{
    FBReq[0] = LE32(4 * 8);
    FBReq[1] = 0;
    FBReq[2] = LE32(0x00028001); /* Set power state */
    FBReq[3] = LE32(8);
    FBReq[4] = LE32(8);
    FBReq[5] = LE32(device_id);
    FBReq[6] = LE32(state);
    FBReq[7] = 0;
    ...
    mbox_recv(8);
    arm_dcache_invalidate((intptr_t)FBReq, 32);

    return LE32(FBReq[6]);
}

/* POC9-STATUS: firmware property helpers used by the USB bare-metal CLI.
 * Return units match Raspberry Pi firmware: millidegrees C and microvolts. */
uint32_t rpi_get_core_temperature(void)
{
    FBReq[0] = LE32(4 * 8);
    FBReq[1] = 0;
    FBReq[2] = LE32(0x00030006); /* Get temperature */
    FBReq[3] = LE32(8);
    FBReq[4] = 0;
    FBReq[5] = 0;               /* temperature id 0 */
    FBReq[6] = 0;
    FBReq[7] = 0;
...
uint32_t rpi_get_core_voltage(void)
{
    FBReq[0] = LE32(4 * 8);
    FBReq[1] = 0;
    FBReq[2] = LE32(0x00030003); /* Get voltage */
    FBReq[3] = LE32(8);
    FBReq[4] = 0;
    FBReq[5] = LE32(1);          /* core voltage */
    FBReq[6] = 0;
    FBReq[7] = 0;
